...
import { WithdrawalCardSelector } from "@/components/withdrawal-card-selector"
import { WithdrawalWizard } from "@/components/withdrawal-wizard"

export type WithdrawalType = "wallet" | "bank" | null
...
export function WithdrawalSheet({ open, onOpenChange }: WithdrawalSheetProps) {
  const [selectedType, setSelectedType] = React.useState<WithdrawalType>(null)
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="flex h-[95vh] flex-col gap-0 rounded-t-xl p-0 [&>button]:hidden">
        <SheetHeader className="flex flex-row items-center justify-between border-b border-border px-6 py-4">
          <SheetTitle className="text-base font-medium">Withdraw</SheetTitle>
        ...
        <div className="flex-1 overflow-y-auto bg-muted p-6">
          {selectedType === null ? (
            <WithdrawalCardSelector onSelect={setSelectedType} />
          ) : (
            <WithdrawalWizard type={selectedType} onBack={handleBack} />
          )}
        </div>
      </SheetContent>
    </Sheet>
